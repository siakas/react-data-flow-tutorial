...
5. **📸 スクリーンショット**: UI変更がある場合は変更前後の画像（ない場合は「N/A」）
6. **🧪 動作確認**: 実施した動作確認項目にチェック
7. **📌 レビュアーへの注意事項**: 特に確認してほしい点（ない場合は「特になし」）
...
4. プルリクエスト作成時には以下を指定：
   ...
   - 本文: **テンプレートの全セクションを完全に記入**（「N/A」や「特になし」も含めて空欄を作らない）
...
- テンプレートのすべてのセクションを記入し、空欄や未記入箇所を残さない
